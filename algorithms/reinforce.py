...
Core idea:
- Calculate returns for each action
- Update policy to make high-return actions more likely 

Policy gradient method thats uses a reward value to adjusts weights 

LOOP:
1. Run episode using current policy
2. For each timestep, calculate return G_t = sum of discounted future rewards
3. Compute loss: -log(π(action|state)) * G_t
4. Update policy network via gradient descent
...
class Policy(tf.keras.Model):
    def __init__(self,hidden_units=128): # 128 layers
